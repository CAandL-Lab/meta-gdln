...
def statistics(forward_params, er_params, gate_params, batch, skey):
  ...
  return jnp.mean(actual_indiv_loss), jnp.mean(actual_sys_loss), jnp.mean(actual_non_loss),\
                                      er_loss, expect_reward, est_loss, gate_probs
...
if __name__ == "__main__":
  ...
  er_layer_sizes = [n1+k1*(2**n1) + gate_layer_sizes[2], num_hidden, 1]
  ...
  for epoch in range(num_epochs):
      ...
      print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$        "+str(i)+"       $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
      key, forward_key, er_key, gating_key, stats_key = random.split(key, num=5)
      forward_params = weight_step(forward_params, gate_params, (X_seen,Y_seen), forward_key)
      er_params = er_step(er_params, gate_params, forward_params, (X_seen,Y_seen), er_key) #(X_val,Y_val)
      gate_params = gate_step(gate_params, er_params, forward_params, (X_seen,Y_seen), gating_key)
      i = ((i + 1) % 20)
            
      print("############################################ Training Metrics #################################################")
      losses[epoch], sys_losses[epoch], non_losses[epoch], er_losses[epoch], expect_reward, exp_losses, used_gates =\
              statistics(forward_params, er_params, gate_params, (X_seen,Y_seen), stats_key) 
      print('Epoch: ',epoch, 'i:',i, ', Loss: ',losses[epoch], ', Sys Loss: ',sys_losses[epoch], ', Non Loss: ',non_losses[epoch],\
            ', ER_Loss: ',er_losses[epoch], '\nExpected Losses\n', exp_losses)

      print("############################################ Test Metrics #################################################")
      test_losses[epoch], sys_test_losses[epoch], non_test_losses[epoch], test_er_losses[epoch], expect_reward, exp_losses, used_gates=\
              statistics(forward_params, er_params, gate_params, (X_test,Y_test), stats_key) 
      print('Epoch: ',epoch, 'i:',i, ', Loss: ',test_losses[epoch], ', Sys Loss: ',sys_test_losses[epoch], ', Non Loss: ',non_test_losses[epoch],\
            ', ER_Loss: ',test_er_losses[epoch], '\nExpected Losses\n',exp_losses)
      #os.system('clear') 
