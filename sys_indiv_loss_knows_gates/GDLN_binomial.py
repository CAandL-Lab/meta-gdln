...
if __name__ == "__main__":
  ...
  for epoch in range(num_epochs):
      ...
      if np.random.rand() < 0.95:
          key, forward_key, er_key, gating_key, stats_key = random.split(key, num=5)
          forward_params = weight_step(forward_params, gate_params, (X_train,Y_train), forward_key)
          er_params = er_step(er_params, gate_params, forward_params, (X_seen,Y_seen), er_key) #(X_val,Y_val)
          gate_params = gate_step(gate_params, er_params, forward_params, (X_val,Y_val), gating_key)
          i = ((i + 1) % 20)
      else:
          key, forward_key, er_key, gating_key, stats_key = random.split(key, num=5)
          forward_params = weight_step(forward_params, gate_params, (X_train,Y_train), forward_key)
