...
def loss(modules_params, gate_params, masks, batch, rand_prob, lkey):
  ...
  keep_probs = jnp.where(gate_probs >= 0.8, 1, 1 - rand_prob)
  keep_action = jax.random.bernoulli(lkey1, keep_probs, gates.shape).astype(np.int16)
...
def er_loss(er_params, modules_params, gate_params, masks, batch, rand_prob, lkey):
  # Loss over a batch of data
  inputs, targets = batch
  gate_probs = gates_predict(gate_params, inputs)
  gates = jnp.where(gate_probs > 0.5, 1, 0)
...
def expected_reward(gate_params, er_params, modules_params, masks, batch, rand_prob, gate_reg_rate, ekey):
...
def statistics(modules_params, er_params, gate_params, masks, batch, rand_prob, skey):
...
if __name__ == "__main__":
  ...
  def weight_step(modules_params, gate_params, masks, batch, rand_prob, key):
    weight_grads = grad(loss)(modules_params, gate_params, masks, batch, rand_prob, key)
    return [[w - weight_step_size * dw for w,dw in zip(modules_params[m], weight_grads[m])] for m in range(len(modules_params))]
 
  def er_step(er_params, gate_params, modules_params, masks, batch, rand_prob, key):
    er_grads = grad(er_loss)(er_params, modules_params, gate_params, masks, batch, rand_prob, key)
    return [w - er_step_size * dw for w, dw in zip(er_params, er_grads)]
 
  def gate_step(gate_params, er_params, modules_params, masks, batch, rand_prob, gate_reg_rate, key):
    indiv_gate_grads = jacrev(expected_reward)(gate_params, er_params, modules_params, masks, batch, rand_prob, gate_reg_rate, key)
  ...
  for epoch in range(warm_up_epochs):
      ...
      er_params = er_step(er_params, gate_params, modules_params, masks, (X,Y), rand_prob, er_key)
      print('Epoch: ', epoch, ' ', str(er_loss(er_params, modules_params, gate_params, masks, (X,Y), rand_prob, stats_key)))
  ...
  for epoch in range(num_epochs):
      ...
      modules_params = weight_step(modules_params, gate_params, masks, (X,Y), rand_prob, module_key)
      er_params = er_step(er_params, gate_params, modules_params, masks, (X,Y), rand_prob, er_key) #(X_val,Y_val)
      gate_params = gate_step(gate_params, er_params, modules_params, masks, (X,Y), rand_prob, gate_reg_rate, gating_key)
      i = ((i + 1) % 20)
      
      print("############################################ Training Metrics #################################################")
      losses[epoch], er_losses[epoch], expect_reward, exp_losses, used_gates, module_enorms[epoch], chosen_enorms[epoch] =\
              statistics(modules_params, er_params, gate_params, masks, (X,Y), rand_prob, stats_key) 
  ...
  plt.colorbar()
  plt.savefig('end_gates/'+str(run_idx)+'_habit.pdf',dpi=400)
  plt.close()
  ...
  plt.grid()
  plt.savefig('losses/'+str(run_idx)+'_habit.pdf')
  plt.close()
 
  norm_names = [r'Common', r'C1 and C2', r'C2 and C3', r'C1 and C3', r'C1', r'C2',r'C3']
  ...
  plt.savefig('train_norms/'+str(run_idx)+'_habit.pdf')
  plt.close()

  np.savetxt('n_runs/'+str(run_idx)+'_habit_train_losses.txt', losses)
  np.savetxt('n_runs/'+str(run_idx)+'_habit_train_module_enorms.txt', chosen_enorms)
