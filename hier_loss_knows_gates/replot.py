...
def plot_losses(losses, labels, colors, linestyles, file_name='losses.pdf', add_legend=True):
    ...
    for i in range(len(mean_losses)):
        plt.plot(mean_losses[i], color=colors[i], label=labels[i], linestyle=linestyles[i])
        plt.fill_between(np.arange(mean_losses.shape[1]), mean_losses[i] - std_losses[i], mean_losses[i] + std_losses[i], color=colors[i], alpha=0.3)
...
def plot_norms(norms, labels, colors, linestyles, file_name='norms.pdf', add_legend=True):
    ...
    for i in range(len(mean_norms)):
        for j in range(mean_norms.shape[2]):
            plt.plot(mean_norms[i,:,j], linestyle=linestyles[i], label=norm_names[j])
            plt.fill_between(np.arange(mean_norms.shape[1]), mean_norms[i,:,j] - std_norms[i,:,j], mean_norms[i,:,j] + std_norms[i,:,j], alpha=0.3)
        plt.axhline(0, color='black')
        plt.axvline(0, color='black')
        #plt.axvline(500, color='black')
        plt.ylabel("Quadratic Loss")
