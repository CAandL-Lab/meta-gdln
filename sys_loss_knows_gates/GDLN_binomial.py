...
if __name__ == "__main__":
  ...
  for epoch in range(num_epochs):
      ...
      if np.random.rand() < 0.95:
          key, module_key, er_key, gating_key, stats_key = random.split(key, num=5)
          modules_params = weight_step(modules_params, modules_ranges, gate_params, (X_train,Y_train), module_key)
          er_params = er_step(er_params, gate_params, modules_params, modules_ranges, (X_seen,Y_seen), er_key)
          gate_params = gate_step(gate_params, er_params, modules_params, modules_ranges, (X_val,Y_val), gating_key)
          i = ((i + 1) % 20)
      else:
          key, module_key, er_key, gating_key, stats_key = random.split(key, num=5)
          modules_params = weight_step(modules_params, modules_ranges, gate_params, (X_train,Y_train), module_key)
  ...
  plt.legend(loc='upper right')
  plt.grid()
  plt.savefig('sys_losses/'+str(run_idx)+'_binomial.pdf')
  plt.close()
  ...
  plt.legend(loc='upper right')
  plt.grid()
  plt.savefig('non_losses/'+str(run_idx)+'_binomial.pdf')
  plt.close()
  ...
  plt.grid()
  plt.legend(loc='upper left')
  plt.savefig('test_norms/'+str(run_idx)+'_binomial.pdf')
  plt.close()

  np.savetxt('n_runs/'+str(run_idx)+'_binomial_train_losses.txt', losses)
  np.savetxt('n_runs/'+str(run_idx)+'_binomial_sys_train_losses.txt', sys_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_binomial_non_train_losses.txt', non_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_binomial_train_module_enorms.txt', chosen_enorms)
  np.savetxt('n_runs/'+str(run_idx)+'_binomial_test_losses.txt', test_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_binomial_sys_test_losses.txt', sys_test_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_binomial_non_test_losses.txt', non_test_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_binomial_test_module_enorms.txt', test_chosen_enorms)  
