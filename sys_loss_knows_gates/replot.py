...
def load_losses():
    names = ['buniform','bbinomial','bhabit','uniform','binomial','habit']
    data = []
    num_runs = 6
    for name in names:
        for run_idx in range(num_runs):
            print('n_runs/'+str(run_idx) + '_' + name + '_train_module_enorms.txt')
            data.append(np.loadtxt('n_runs/'+str(run_idx) + '_' + name + '_test_losses.txt'))
            print(data[-1].shape)
...
def load_norms():
    ...
    for name in names:
        for run_idx in range(num_runs):
            data.append(np.loadtxt('n_runs/'+str(run_idx) + '_' + name + '_train_module_enorms.txt'))
    return np.array(data).reshape(len(names), num_runs, data[0].shape[0], data[0].shape[1]), names

def load_mds():
    data = np.loadtxt('n_runs/10_binomial_mds.txt')
    data_reshape = np.array(data).reshape(num_samples, 32, 60*9)
    return data_reshape
...
def plot_losses(losses, labels, colors, linestyles, file_name='losses.pdf', add_legend=True):
    ...
    for i in range(len(mean_losses)):
        plt.plot(mean_losses[i][:2000], color=colors[i], label=labels[i], linestyle=linestyles[i])
        plt.fill_between(np.arange(2000), mean_losses[i][:2000] - std_losses[i][:2000],\
                                          mean_losses[i][:2000] + std_losses[i][:2000], color=colors[i], alpha=0.3)
...
def plot_norms(norms, labels, colors, linestyles, file_name='norms.pdf', add_legend=True):
    mean_norms = np.mean(norms, axis=1)
    std_norms = np.std(norms, axis=1)
    norm_names = [r'$\Omega_{X} \Omega_{Y}$', r'$\Omega_{X} \Gamma_{Y}$', r'$\Omega_{X} Y$', r'$\Gamma_{X} \Omega_{Y}$', r'$\Gamma_{X} \Gamma_{Y}$', r'$\Gamma_{X} Y$', r'$X \Omega_{Y}$',r'$X \Gamma_{Y}$',r'$XY$']
    for i in range(len(mean_norms)):
        for j in range(mean_norms.shape[2]):
            plt.plot(mean_norms[i,:2000,j], linestyle=linestyles[i], label=norm_names[i])
            plt.fill_between(np.arange(2000), mean_norms[i,:2000,j] - std_norms[i,:2000,j], mean_norms[i,:2000,j] + std_norms[i,:2000,j], alpha=0.3)
        plt.axhline(0, color='black')
        plt.axvline(0, color='black')
        #plt.axvline(500, color='black')
        plt.ylabel("Quadratic Loss")
        ...
        if add_legend:
            plt.legend()
        plt.tight_layout()
        plt.savefig('plots/'+labels[i]+'_'+file_name, dpi=400, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    if loss:
        ...
        labels = ['Baseline Uniform','Baseline Binomial','Baseline Habit','Uniform','Binomial','Habit']
        colors = ['deeppink','blue','green','deeppink','blue','green']
        linestyles = ['--','--','--','-','-','-']
        plot_losses(losses, labels, colors, linestyles, file_name='plots/losses_test.pdf', add_legend=True)
    
    if norms:
        norms, file_names = load_norms()
        labels = ['Baseline Uniform','Baseline Binomial','Baseline Habit','Uniform','Binomial','Habit']
        colors = ['deeppink','blue','green','deeppink','blue','green']
        linestyles = ['--','--','--','-','-','-']
        plot_norms(norms, labels, colors, linestyles, file_name='norms.pdf', add_legend=True)

    if mds:
        sample_size = 10
        jump_size = 2 #10
        num_samples = int(2000/sample_size)+1
