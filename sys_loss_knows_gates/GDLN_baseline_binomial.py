...
def predict_hidden(modules_params, modules_ranges, inputs, gates, key):
  output = jnp.zeros((num_hidden*9, inputs.shape[1]))
  for m in range(len(modules_params)):
      module_in_range, module_out_range = modules_ranges[m]
      output = output.at[num_hidden*m:num_hidden*(m+1)].set(\
               output[num_hidden*m:num_hidden*(m+1)] +\
               gates[m]*module_predict_hidden(modules_params[m], inputs[module_in_range[0]:module_in_range[1]]))
  return output

def loss(modules_params, modules_ranges, gate_params, batch, lkey):
...
def er_loss(er_params, modules_params, modules_ranges, gate_params, batch, lkey):
  # Loss over a batch of data
  inputs, targets = batch
  gate_probs = gates_predict(gate_params, inputs)
  gates = jnp.where(gate_probs > 0.5, 1, 0)
...
def expected_reward(gate_params, er_params, modules_params, modules_ranges, batch, ekey):
...
def statistics(modules_params, modules_ranges, er_params, gate_params, batch, skey):
...
if __name__ == "__main__":
  ...
  def weight_step(modules_params, modules_ranges, gate_params, batch, key):
    weight_grads = grad(loss)(modules_params, modules_ranges, gate_params, batch, key)
    return [[w - weight_step_size * dw for w,dw in zip(modules_params[m], weight_grads[m])] for m in range(len(modules_params))]
 
  def er_step(er_params, gate_params, modules_params, modules_ranges, batch, key):
    er_grads = grad(er_loss)(er_params, modules_params, modules_ranges, gate_params, batch, key)
    return [w - er_step_size * dw for w, dw in zip(er_params, er_grads)]
 
  def gate_step(gate_params, er_params, modules_params, modules_ranges, batch, key):
    indiv_gate_grads = jacrev(expected_reward)(gate_params, er_params, modules_params, modules_ranges, batch, key)
  ...
  er_layer_sizes = [n1+k1*(2**n1) + gate_layer_sizes[2], 100, 1]
  ...
  num_epochs = 8000 #8000
  warm_up_epochs = 1000 #2000
  mds_sample_rate = 10
  ...
  mds = np.zeros( (int(num_epochs/mds_sample_rate)+1,int(num_hidden*9),X_seen.shape[1]) )
  ...
  for epoch in range(warm_up_epochs):
      ...
      er_params = er_step(er_params, gate_params, modules_params, modules_ranges, (X_seen,Y_seen), er_key)
      print('Epoch: ', epoch, ' ', str(er_loss(er_params, modules_params, modules_ranges, gate_params, (X_seen,Y_seen), stats_key)))
  ...
  for epoch in range(num_epochs):
      ...
      print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$        "+str(i)+"       $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
      key, module_key, er_key, gating_key, stats_key = random.split(key, num=5)
      modules_params = weight_step(modules_params, modules_ranges, gate_params, (X_seen,Y_seen), module_key)
      er_params = er_step(er_params, gate_params, modules_params, modules_ranges, (X_seen,Y_seen), er_key)
      gate_params = gate_step(gate_params, er_params, modules_params, modules_ranges, (X_seen,Y_seen), gating_key)
      i = ((i + 1) % 20)
      ...
      losses[epoch], sys_losses[epoch], non_losses[epoch], er_losses[epoch], expect_reward, exp_losses,\
              ...
              statistics(modules_params, modules_ranges, er_params, gate_params, (X_seen,Y_seen), stats_key) 
      print('Epoch: ',epoch, 'i:',i, ', Loss: ',losses[epoch], ', Sys Loss: ',sys_losses[epoch], ', Non Loss: ',non_losses[epoch],\
            ', ER_Loss: ',er_losses[epoch], '\nExpected Losses\n', exp_losses,\
            ',\nModules Chosen:\n',chosen_enorms[epoch], ',\nModules Used:\n',module_enorms[epoch])
      ...
      test_losses[epoch], sys_test_losses[epoch], non_test_losses[epoch], test_er_losses[epoch], expect_reward, exp_losses,\
              ...
              statistics(modules_params, modules_ranges, er_params, gate_params, (X_test,Y_test), stats_key) 
      print('Epoch: ',epoch, 'i:',i, ', Loss: ',test_losses[epoch], ', Sys Loss: ',sys_test_losses[epoch],\
            ', Non Loss: ',non_test_losses[epoch], ', ER_Loss: ',test_er_losses[epoch],\
            '\nExpected Losses\n',exp_losses,',\nModules Chosen:\n',test_chosen_enorms[epoch],',\nModules Used:\n',test_module_enorms[epoch])
      #os.system('clear')
      if (epoch % mds_sample_rate) == 0:
          gate_probs = gates_predict(gate_params, X_seen)
          gates = jnp.where(gate_probs > 0.5, 1, 0)
          preds_hidden = predict_hidden(modules_params, modules_ranges, X_seen, gates, stats_key)
  ...
  plt.colorbar()
  plt.savefig('end_gates/'+str(run_idx)+'_bbinomial.pdf',dpi=400)
  plt.close()
  ...
  plt.grid()
  plt.savefig('losses/'+str(run_idx)+'_bbinomial.pdf')
  plt.close()
  ...
  plt.legend(loc='upper right')
  plt.grid()
  plt.savefig('sys_losses/'+str(run_idx)+'_bbinomial.pdf')
  plt.close()
  ...
  plt.legend(loc='upper right')
  plt.grid()
  plt.savefig('non_losses/'+str(run_idx)+'_bbinomial.pdf')
  plt.close()
  ...
  plt.savefig('train_norms/'+str(run_idx)+'_bbinomial.pdf')
  plt.close()
  ...
  plt.grid()
  plt.legend(loc='upper left')
  plt.savefig('test_norms/'+str(run_idx)+'_bbinomial.pdf')
  plt.close()

  np.savetxt('n_runs/'+str(run_idx)+'_bbinomial_train_losses.txt', losses)
  np.savetxt('n_runs/'+str(run_idx)+'_bbinomial_sys_train_losses.txt', sys_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_bbinomial_non_train_losses.txt', non_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_bbinomial_train_module_enorms.txt', chosen_enorms)
  np.savetxt('n_runs/'+str(run_idx)+'_bbinomial_test_losses.txt', test_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_bbinomial_sys_test_losses.txt', sys_test_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_bbinomial_non_test_losses.txt', non_test_losses)
  np.savetxt('n_runs/'+str(run_idx)+'_bbinomial_test_module_enorms.txt', test_chosen_enorms)
